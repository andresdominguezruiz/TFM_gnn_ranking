...
import torch
from exp_layer_type.conv_pagerank import CNN_PageRank
from exp_layer_type.gat_pagerank import GAT_PageRank
from exp_layer_type.sage_pagerank import GSAGE_PageRank
from utils import *
import random
import torch.nn as nn
from model_pagerank import GNN_PageRank
...
print(gtype)
if gtype == "SF":
    data_path = "./datasets/data_splits/SF/eigen/"
    print("Scale-free graphs selected.")

elif gtype == "ER":
    data_path = "./datasets/data_splits/ER/eigen/"
...
list_adj_train,list_adj_mod_train = graph_to_adj_eigen(list_graph_train,list_n_seq_train,list_num_node_train,model_size)
list_adj_test,list_adj_mod_test = graph_to_adj_eigen(list_graph_test,list_n_seq_test,list_num_node_test,model_size)
...
model=None
if gnn_type=="GNN":
    model = GNN_PageRank(ninput=model_size,nhid=hidden,dropout=0.6,num_intermediate_layers=num)
elif gnn_type=="CNN":
    model = CNN_PageRank(ninput=model_size,nhid=hidden,dropout=0.6,num_intermediate_layers=num)
elif gnn_type=="GAT":
    model = GAT_PageRank(ninput=model_size,nhid=hidden,dropout=0.6,num_intermediate_layers=num)
elif gnn_type=="SAGE":
    model = GSAGE_PageRank(ninput=model_size,nhid=hidden,dropout=0.6,num_intermediate_layers=num)
...
with open(f"results/page_rank/{model.get_num_intermediate_layers()}_{model.get_gnn_type()}_{gtype}{optional}{v}_kt.pickle","wb") as fopen2:
