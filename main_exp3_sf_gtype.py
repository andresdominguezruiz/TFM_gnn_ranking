...
for i,j in zip(sf_values,index_values):
    # Construir el comando con el valor de i
    print(f"###########BETWEENNESS PARA {G_TYPE} , CON {HYPER} = {i} ################")
    #RESETEO DE GRAFOS#--------------------------------------
    subprocess.run(f"python central_generate_graph.py --num_graphs 15 --min_nodes 5000 --max_nodes 10000 --g_type {G_TYPE} --{HYPER} {i} --centrality bet", shell=True,cwd="datasets")
    subprocess.run(f"python central_create_dataset.py --split_train 5 --split_test 10 --model_size 10000 --num_copies 20 --g_type {G_TYPE} --centrality bet" ,shell=True,cwd="datasets")

    comando_bet = f"python betweenness.py --g {G_TYPE} --num_intermediate_layer 5 --model_size 10000 --optional_name gamma{HYPER}{j} --g_hype {i}"
